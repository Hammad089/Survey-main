...
const SvrDraft = () => {
  ...
  const shops = [
    ...
    const uploadDataSvr = async (surveyData,surveys) => {
      console.log('Debug: surveyData', surveyData);
      console.log('Debug: surveys', surveys.images);
      try {
        ...
        console.log('ImgUrl:', surveyData?.surveys?.images?.ImgUrl);
        if (!surveyData?.surveys?.images?.ImgUrl && surveyData?.ShopNameActual === 'No') {
          // Alert.alert('Front image is mandatory. Please take a picture before submitting.');
          console.log('Debug: ShopNameActual:', surveyData?.ShopNameActual);
          console.log('Debug: ImgUrl:', surveyData?.images?.ImgUrl);
          return;
        }
      
        if (surveyData?.ShopNameActual === 'No' && !surveyData?.surveys?.images?.ImgUrl) {
          Alert.alert('Please take the front image before submitting.');
          return;
      }

      if (surveyData?.ShopTypeActual === 'No' && !surveyData?.surveys?.images?.insideImgUrl) {
          Alert.alert('Inside picture is mandatory when ShopProfile is "No". Please take a picture before submitting.');
          return;
      }

      if (surveyData?.ShopGPSLocation === 'No' && !surveyData?.surveys?.images?.gpsLocationImage) {
    ...
    const uploadImage = async (surveyData,surveys) => {
      console.log('surveys in image',surveys)
      try {
          const token = await AsyncStorage.getItem('token');
          setLoading(true);
          ...
          // Append front image
          if (surveyData?.surveys?.images?.ImgUrl) {
              formData.append('front_image', {
                  uri: surveyData?.surveys?.images?.ImgUrl,
                  type: 'image/jpeg',
          ...
          // Append inside image
          if (surveyData?.surveys?.images?.insideImgUrl) {
              formData.append('inside_image', {
                  uri: surveyData?.surveys?.images?.insideImgUrl,
                  type: 'image/jpeg',
          ...
          if (surveyData?.surveys?.images?.gpsLocationImage) {
              formData.append('current_location_image', {
                  uri: surveyData?.surveys?.images?.gpsLocationImage,
                  type: 'image/jpeg',
          ...
          // Append other images
          const imageUrls = [surveyData?.images?.ImgUrl1, surveyData?.images?.ImgUrl2, surveyData?.images?.ImgUrl3, surveyData?.images?.ImgUrl4, surveyData?.images?.ImgUrl5];
  ...
  useEffect((surveyData) => {
    if (surveyId) {
      uploadImage(surveyData);
    }
  }, [surveyId]);
  return (
    ...
    // <Image key={index} style={{ width: 100, height: 100, margin: 5 }} source={{ uri: imageUrl }} />
          ...
          {retrievedSurveyData.map((survey, index) => (
            <View style={styles.tableRow} key={index}>
              <Text style={styles.tableCell}>{survey.surveyData.supervisorName}</Text>
              ...
              <TouchableOpacity onPress={() => uploadDataSvr(survey.surveyData)}>
